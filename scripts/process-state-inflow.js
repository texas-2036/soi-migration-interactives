...
const fs = require('fs');
const d3 = require('d3')
const outflow = fs.readFileSync(__dirname + '/../src/data/stateinflow1516.csv').toString();
...
let filtered = parsed.filter((d) => +d.y2_statefips === 48 && d.y1_state !== 'TX' && d.y1_state !== 'FR');
const filteredOld = parsedOld.filter((d) => +d.y2_statefips === 48 && d.y1_state !== 'TX' && d.y1_state !== 'FR');

const oldMap = {};
filteredOld.forEach((d) => {
  oldMap[+d.y1_statefips] = d;
})
...
filtered = filtered.map(d => {
  return { ...d, lastYear: oldMap[+d.y1_statefips] }
});

console.log(filtered);

fs.writeFileSync(__dirname + '/../src/data/state-inflow-filtered.json', JSON.stringify(filtered));
